...
- サイト固有の作り（求人の絞り方・部署別 仕事ガイド・2つの入口）は **`CLAUDE.md` の冒頭**にまとまっている。
  以下は jobsite から引き継いだ共通部分の説明で、**サイト名とドメイン以外はそのまま当てはまる**。

## フォルダ構成
- `robots.txt` / `sitemap.xml` … 独自ドメインなので agent-best.net 側のものが効かない。ここに直接置いている（求人詳細 `?job=` は jobsite と同じ求人なので sitemap に入れない）
